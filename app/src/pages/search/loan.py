import sys
import os
from PyQt5 import uic
from PyQt5.QtWidgets import QMainWindow, QApplication, QMessageBox, QTableWidgetItem
from PyQt5.QtGui import QStandardItemModel, QStandardItem, QIcon
from PyQt5.QtCore import Qt, QDate
from datetime import datetime
import traceback

from src.components import DB  # Firestore 연결을 위한 모듈

class SearchLoanApp(QMainWindow):
    def __init__(self):
        super().__init__()
        current_dir = os.path.dirname(os.path.abspath(__file__))
        ui_path = os.path.join(current_dir, "loan.ui")  # UI 파일 경로 (고객상담자료검색)
        uic.loadUi(ui_path, self)

        icon_path = os.path.join(current_dir, 'icon.ico')
        self.setWindowIcon(QIcon(icon_path))

        # 창 크기 고정
        self.setFixedSize(self.size())

        self.StartDate.setDate(QDate.currentDate())
        self.LastDate.setDate(QDate.currentDate())

        self.setup_connections()
        self.setup_loan_table()

    def setup_connections(self):
        # 검색 버튼 클릭 연결
        self.SearchButton.clicked.connect(self.search_loan_data)

    def setup_loan_table(self):
        # 테이블의 초기 설정 (TableView에 사용할 모델 설정)
        self.model = QStandardItemModel(0, 5)  # 5열 테이블로 설정 (customer_name, nrc_no, loan_number, loan_type, loan_officer)
        self.model.setHorizontalHeaderLabels(["Customer Name", "NRC No.", "Loan Number", "Loan Type", "Loan Officer"])
        self.loanTable.setModel(self.model)
        # 데이터가 없을 때는 컬럼 크기 조정 방지
        self.loanTable.horizontalHeader().setStretchLastSection(True)
    def search_loan_data(self):
        try:
            # 사용자 입력 데이터 가져오기
            customer_name = self.CustomerName.toPlainText().strip()
            start_date = self.StartDate.date().toPyDate()  # QDateEdit에서 날짜 가져오기
            end_date = self.LastDate.date().toPyDate()  # QDateEdit에서 날짜 가져오기
            # 입력된 고객명을 Customer DB에서 검색
            if customer_name:
                customer_ref = DB.collection('Customer').where("name", "==", customer_name).get()

                if not customer_ref:
                    QMessageBox.warning(self, "No Data", "No customer found with the given name.")
                    return

                # 고객명에 해당하는 고객의 UID 가져오기
                customer_id = customer_ref[0].id  # 첫 번째 일치 항목의 UID 사용

                # Loan DB에서 해당 고객의 대출 정보 검색
                loan_ref = DB.collection('Loan').where("uid", "==", customer_id).get()

                # Loan 정보를 테이블에 채우기
                self.populate_table(loan_ref, customer_id, start_date, end_date)
            else:
                QMessageBox.warning(self, "Input Error", "Please enter a customer name.")

        except Exception as e:
            QMessageBox.critical(self, "Error", "An error occurred while searching for customer data.")
            print(f"Error: {e}")
            print(traceback.format_exc())  # 상세 오류 정보 콘솔 출력

    def get_customer_data(self, customer_id):
        """
        Customer DB에서 customer_id를 사용하여 customer_name과 nrc_no를 가져오는 함수
        """
        try:
            customer_ref = DB.collection('Customer').document(customer_id)
            customer_doc = customer_ref.get()

            if customer_doc.exists:
                customer_data = customer_doc.to_dict()
                return customer_data.get("name", ""), customer_data.get("nrc_no", "")
            else:
                return "", ""
        except Exception as e:
            print(f"Error fetching customer data: {e}")
            return "", ""

    def populate_table(self, loan_results, customer_id, start_date, end_date):
        # 기존 테이블 데이터 삭제
        self.model.removeRows(0, self.model.rowCount())

        # 검색 결과가 없음을 확인하기 위한 플래그
        data_found = False

        # 고객 정보를 Customer DB에서 가져오기
        customer_name, nrc_no = self.get_customer_data(customer_id)

        # Loan 데이터가 있을 경우 테이블에 표시
        try:
            # loan_results 출력해서 확인
            print(f"Loan results: {loan_results}")

            for doc in loan_results:
                loan_data = doc.to_dict()
                contract_date_str = loan_data.get("contract_date", "")

                # Firestore에서 받은 contractDate가 비어있는지 확인
                if not contract_date_str:
                    print("No contractDate found")
                    continue

                # contractDate를 datetime 객체로 변환
                try:
                    contract_date = datetime.strptime(contract_date_str, "%Y-%m-%d").date()
                except ValueError:
                    print(f"Invalid contractDate format: {contract_date_str}")
                    continue

                # 계약일자를 기준으로 필터링
                if start_date <= contract_date <= end_date:
                    loan_number = loan_data.get("loan_number", "")
                    loan_type = loan_data.get("loan_type", "")
                    loan_officer = loan_data.get("loan_officer", "")

                    # 데이터를 테이블에 추가
                    row_data = [
                        QStandardItem(customer_name),
                        QStandardItem(nrc_no),
                        QStandardItem(loan_number),
                        QStandardItem(loan_type),
                        QStandardItem(loan_officer)
                    ]
                    self.model.appendRow(row_data)
                    data_found = True  # 데이터가 있음을 표시

            # 테이블이 다시 그려지도록 강제 업데이트
            self.loanTable.setModel(self.model)
            self.loanTable.resizeColumnsToContents()

            # 만약 데이터를 찾지 못했다면 경고 메시지 표시
            if not data_found:
                QMessageBox.warning(self, "No Data", "No loan data found for the specified customer.")

        except Exception as e:
            QMessageBox.critical(self, "Error", "An error occurred while processing loan data.")
            print(f"Error: {e}")
            print(traceback.format_exc())  # 상세 오류 정보 콘솔 출력


def main():
    app = QApplication(sys.argv)
    window = SearchLoanApp()
    window.show()
    sys.exit(app.exec_())


if __name__ == "__main__":
    main()